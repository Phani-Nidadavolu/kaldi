...
splice_ind="-3,-2,-1,0,1,2,3 -1,0,1,2 -3,-2,-1,0,1,2,3 -7,-6,-5,-4,-3,-2,-1,0,1,2 0"
...
dir=exp/tdnn_reg/$gen/${feattype}_lab_testfold_${foldnum}_hl_${hl}_reludim_${relu_dim}
...
cp $data_dir/train/feats_lab_age.scp $targets_scp
...
if [ $stage -le 13 ]; then
  ...
  for x in test train; do
    ...
    [ ! -f $targets_scp ] && echo missing:$targets_scp && exit 1;
    matrix-sum --scale2=-1 scp:${targets_scp} scp:${nnet3dir}/fwd/$x/out.scp ark,scp:$nnet3dir/fwd/$x/error.ark,$nnet3dir/fwd/$x/error.scp 
  done
...
if [ $stage -le 14 ]; then
  ...
  for x in test train; do
      python compute_weighted_mae.py "$nnet3dir/fwd/$x/error.scp" || exit 1;
  done
fi
