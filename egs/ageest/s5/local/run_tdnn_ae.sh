...
dir=exp/tdnn_reg/$gen/ae_${feattype}_testfold_${foldnum}_hl_${hl}_reludim_${relu_dim}

data_dir=data/$gen/${feattype}_test_fold${foldnum}
targets_scp=$dir/labels.scp

mkdir -p $dir
touch ${dir}/splice_indexes && echo $splice_ind > ${dir}/splice_indexes

cp $data_dir/train/feats.scp $targets_scp
...
if [ $stage -le 13 ]; then
  ...
  for x in test train; do
    ...
    matrix-sum --scale2=-1 scp:${data_dir}/${x}/feats.scp scp:${nnet3dir}/fwd/$x/out.scp ark,scp:$nnet3dir/fwd/$x/error.ark,$nnet3dir/fwd/$x/error.scp 
  done
