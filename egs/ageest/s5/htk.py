...
def readhtk_segment(segment, lc=0, rc=0, return_parmKind_and_sampPeriod=False):
...
def readhtk_segment_old(file, start, end, return_parmKind_and_sampPeriod=False):
    """ Read segment from htk feature file
     Input:
...
def lab_to_frm_old(labels, length=0, frame_rate=100., empty_label=None):
    min_len, max_len = (length, length) if length > 0 else (-length, None)
    start, end = np.rint(frame_rate/1e7*labels.T[:2].astype(int)).astype(int)
    if not end.size:
...
def lab_to_frm(segment,mlf_dict,frame_rate=100., empty_label=None):
...
def readScp(scp_file):
  ...
  return np.array([(l,f,int(s),int(e)+1) for l,f,s,e in [re.search(r'(.*)=(.*)\[(.*),(.*)]', line).groups()
                                           for line in np.loadtxt(scp_file, dtype=object)]], dtype=object)
